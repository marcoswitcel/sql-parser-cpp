...
Binary_Expression_Ast_Node* SQL_Parse_Context::eat_binary_expression_ast_node()
{
  ...
  while (!node->left || !node->right || node->op == Binary_Operation::Unknown)
  {
    ...
    {
      node->op = Binary_Operation::Equals;
    }
    ...
    {
      node->op = Binary_Operation::Greater_Than;
    }
    else
    {
...
static inline bool try_consume_keyword(SQL_Parse_Context *parser, std::string token_identifier)
{
  ...
  auto char_after_last_char = parser->peek_n_char(index_after_last_char);
  
  if (equal && (
      char_after_last_char == END_OF_SOURCE ||
      parser->is_whitespace(char_after_last_char) ||
      Is_Expression_Terminator(char_after_last_char)
    )
  )
  {
    for (size_t i = 0; i < token_identifier.size(); i++)
...
void try_parse_ident(SQL_Parse_Context* parser, Token *token, bool *success)
{
  ...
  while (c != END_OF_SOURCE && !Is_Expression_Terminator(c))
  {
    ...
    // @note João, ainda não lida com áspas duplas no meio de um ident com "quote",
    // também vai bugar com ',' e os outros terminadores de expressão no meio de sequência
    // 'quotadas'. Buga também com "..." no meio das sequências 'quotadas', isso porque
    // não é um caractere válido... Decidir o que fazer
